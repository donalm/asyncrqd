...
if __name__ == "__main__":
    if os.name == 'nt':
        ...
        child_watcher = ResourceUsageSafeChildWatcher()
    ...
    with contextlib.closing(loop):
        ...
        e0 = os.environ.copy()
        e0["cmd_id"] = "0"
        soh0 = SubprocessOutputHandler("/tmp/donal_test_log.000.log")
        soh0.connect_fh(sys.stdout)
        sp0 = SubProcess(['/home/donal/Geek/asyncrqd/bin/hurtme', "5"], soh0, nice=10, cpu_list_arg="0,2", env=e0, cwd="/tmp/p0")
        f0 = sp0.spawn(loop, soh0)

        e1 = os.environ.copy()
        e1["cmd_id"] = "1"
        soh1 = SubprocessOutputHandler("/tmp/donal_test_log.001.log")
        soh1.connect_fh(sys.stdout)
        sp1 = SubProcess(['/home/donal/Geek/asyncrqd/bin/hurtme', "15"], soh1, nice=10, cpu_list_arg="1,3", env=e1, cwd="/tmp/p1")
        f1 = sp1.spawn(loop, soh1)

        e2 = os.environ.copy()
        e2["cmd_id"] = "2"
        soh2 = SubprocessOutputHandler("/tmp/donal_test_log.002.log")
        soh2.connect_fh(sys.stdout)
        sp2 = SubProcess(['/home/donal/Geek/asyncrqd/bin/hurtme', "20"], soh2, nice=10, cpu_list_arg="1,3", env=e2, cwd="/tmp/p2")
